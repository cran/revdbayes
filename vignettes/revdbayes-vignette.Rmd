...
distributions using the function `rpost`.  From version 1.2.0 onwards sampling can be achieved more quickly using the function `rpost_rcpp`. See the vignette [Faster simulation using revdbayes](revdbayes-using-rcpp-vignette.html) for details.

Performing posterior predictive extreme value inference using posterior samples
is covered in a separate vignette
[Posterior Predictive Extreme Value Inference using the revdbayes Package](revdbayes-predictive-vignette.html)
...
offset the extra time needed to compute the posterior density.  
...
Repeating these analyses for a much higher threshold, set at the 95\% sample
